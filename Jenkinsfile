//ldop-ldap-phpadmin/Jenkinsfile
...
pipeline {
    ...
    stages {
        ...
        stage('ldop-ldap-phpadmin-validate') {
            agent any
            steps {
                ...
                script {
                    ...
                    if (doesVersionExist('liatrio', 'ldop-ldap-phpadmin', "${TAG}")) {
                        error("LDOP LDAP PHP Admin version already exists! Aborting...")
                    }
                }
            }
            post {
                ...
                always { script { SUBJECT = "Build #${env.BUILD_NUMBER} of ${env.JOB_NAME} at 'ldop-ldap-phpadmin-validate'" } }
            }
        }
        stage('ldop-ldap-phpadmin-build') {
            agent any
            steps {
                script { CHANGED = "NO" }
                sh "docker build -t liatrio/ldop-ldap-phpadmin:${env.BRANCH_NAME} ."
                sh "docker push liatrio/ldop-ldap-phpadmin:${env.BRANCH_NAME}"
            }
            post {
                success {
                    script { STATUS = "SUCCESS" }
                ...
                always { script { SUBJECT = "Build #${env.BUILD_NUMBER} of ${env.JOB_NAME} at 'ldop-ldap-phpadmin-build'" } }
            }
        }
        stage('ldop-integration-testing') {
            ...
            steps {
                ...
                testSuite("ldop-ldap-phpadmin", "${TAG}", "${DIR}")
        ...
        stage('ldop-image-deploy') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-ldap-phpadmin:${env.BRANCH_NAME} liatrio/ldop-ldap-phpadmin:${TAG}"
                sh "docker push liatrio/ldop-ldap-phpadmin:${TAG}"
            }
            post {
                success {
                    script { STATUS = "SUCCESS" }
